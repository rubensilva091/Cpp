How to open:

1. install cmake
2. open the folder "engine"
3. create inside that folder ".../engine/build"
4. run the solution

Videos do show the final result:

Phase2:https://www.youtube.com/watch?v=TRg4daqbpBo&ab_channel=RubenSilva
Phase3:https://www.youtube.com/watch?v=zl5xRsTnnA4&ab_channel=RubenSilva
Phase4:https://www.youtube.com/watch?v=Uy5wZQZ3jas&ab_channel=RubenSilva

NOTE: 20/20 University of UMinho